...
import { Demandas } from 'src/app/models/demandas';
...
export class DemandasComponent {
  ...
  public Items: Demandas[]=[];
  public Items2: Gestiones_Demandas[]=[];
  public ItemsALL: Demandas[]=[];
...
eliminarTiposdetalle (tptiposdetalle2: Demandas)
{
//console.log(tptiposdetalle2);
Swal.fire({
title: '¿Borrar Demandas?',
...
public camposEditar : Demandas=new Demandas(0,'',0,0,0,'','',new Date(),'','','','',new Date(),'', new Date(), '','',new Date(),new Date(),'','',0,new Date(),'',0,new Date(),'','',0,0,'','','','','','','','','','','','','',new Date(),'',0,0,'','');


Titulo: string="Demandas";
...
this.camposEditar =new Demandas(0,'',0,0,0,'','',new Date(),'','','','',new Date(),'', new Date(), '','',new Date(),new Date(),'','',0,new Date(),'',0,new Date(),'','',0,0,'','','','','','','','','','','','','',new Date(),'',0,0,'','');

this.abrirModal();
}

abrirModificar(dtiposdetalle:Demandas){
this._Crear=false;
...
let  nueva    : Demandas =new Demandas(0,'',0,0,0,'','',new Date(),'','','','',new Date(),'', new Date(), '','',new Date(),new Date(),'','',0,new Date(),'',0,new Date(),'','',0,0,'','','','','','','','','','','','','',new Date(),'',0,0,'','');
...
public camposEditarTarea : Gestiones_Demandas=new Gestiones_Demandas(0,'',0,0,new Date(),'',new Date(),'','','',1,0,'','','','','','');
...
abrirModalTarea(Gestiondetalle:Demandas){

this.camposEditarTarea =new Gestiones_Demandas(0,'',Gestiondetalle.coddeman,0,new Date(),'',new Date(),'','','',1,0,'','','','','','');
...
{
        //crear nueva tarea
        // let  nueva    : Gestiones_Demandas =  new Gestiones_Demandas(0,'',this.Empresa,'','',this.camposEditarTarea.tarea,this.fechaVenceTarea,1,'',this.camposEditarTarea.idgestion,this.camposEditarTarea.idexpediente,0,0,'','');

        // console.log(nueva);
        // console.log("nueva");
        // console.log(this.fechaVenceTarea);

        // this.gestionesService.crear(nueva)
        // .subscribe(resp =>
        // {
        //   this.Logs = JSON.stringify(resp);
...
//modificar tarea
    ...
    // this.gestionesService.modificar(nueva)
    // .subscribe(resp =>
    // {
    // this.Logs = JSON.stringify(resp);
...
abrirModalGestion2(Gestiondetalle:Demandas)
{

    let coddeman = Gestiondetalle.coddeman;
 

    this.cargarGestion(coddeman );
...
cargarGestion(coddeman :number=0) {

// this.cargando = true;
console.log("GESTIONE");
console.log(coddeman);

this.gestionesService.cargar(coddeman)
.subscribe ( (res1:any) => 
{
this.Items2= res1['resultado'];
// this.totalTipos=res1.total;
// this.paginasTotales= Math.round(this.totalTipos/this.limite);
// this.paginaActual=  this.desde+1;
